...
void setSpeed(int leftSpeed, int rightSpeed) {
    ...
    svQEL = leftSpeed / 9; // Don't know this
    ...
    svQER = rightSpeed / 9; // Don't know this
}    
...
void movePoint(int x_des, int y_des, int angle, char mode){
	...
	// Angle calculation
    rb_angle = atan2(robotctrl.oy, robotctrl.ox);
    des_angle = atan2(vy_des,vx_des);
...
void LEDLtoggle()
{
    if(LEDL==0){LEDL=1;}else{LEDL=0;}
}

void LEDRtoggle()
{
    if(LEDR==0){LEDR=1;}else{LEDR=0;}
}
